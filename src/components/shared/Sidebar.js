...
const Sidebar = (props) => {
    ...
    return (<div className={`bg-white duration-300 flex flex-col fixed h-full overflow-x-hidden top-0 left-0`} ref={sidebarRef} style={{ width: props.collapse ? "220px" : "55px" }}>
        ...
        </div>
        {
            menus.map((menu, key) => (
                <Link className="duration-150 flex font-bold group items-center py-2 px-3 text-gray-700 hover:bg-gray-100" key={key} to={menu.url}>
                    ...
                    <span className="pl-[16px]">{menu.name}</span>
                </Link>
            ))
        }
    </div>)
